...
import {
  LayoutDashboard, Map, History, FileText, Settings, ChevronLeft, ChevronRight, Sun, LogOut
} from 'lucide-react';
...
export function Sidebar() {
  ...
  return (
    ...
    >
      ...
      {/* User */}
      <div className="p-3 border-t border-white/5 flex-shrink-0 space-y-3">
          ...
          <AnimatePresence>
            {!collapsed && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="overflow-hidden flex-1">
                <div className="text-xs font-bold text-white whitespace-nowrap truncate">{user?.username || 'User'}</div>
